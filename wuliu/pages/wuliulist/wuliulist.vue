<template>
	<view>
		<view style="display: flex;line-height: 80rpx;background-color: #fff;width: 750rpx;padding: 31rpx;margin: 0;">
			<u-search placeholder="根据用户名搜索物流单" :show-action="false" @search="getdata"></u-search>
		</view>
		<view class="" v-if="list.length ==0" style="text-align: center;line-height: 100rpx;color: #aaa;">暂无相关物流单数据</view>
		<view>
			
			<u-card v-for="item in list" v-if="userid == item.user_id || userlimits == 1">
				<view slot="head">
					...
					<uni-table stripe emptyText="暂无更多数据">
						...
						<uni-tr>
							...
							<uni-td>
								...
								<u-button style="background-color: #007aff;color: #fff;height: 60rpx;width: 150rpx;" v-if="item.isservice == 0" @click="sendarr(item.id)">取消寄件</u-button>
								<u-button style="background-color: #90cfff;color: #fff;height: 60rpx;width: 150rpx;" v-if="item.isservice == 1">已取消</u-button>
								<u-button style="background-color: #90cfff;color: #fff;height: 60rpx;width: 150rpx;" v-if="item.isservice == 2">已送达</u-button>
							</uni-td>			
						</uni-tr>
					</uni-table>
				</view>
	...
	export default {
		data() {
			return {
				...
				userlimits:uni.getStorageSync("user").limits
			}
		},
		onLoad(){
			this.getdata("")
