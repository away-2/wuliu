<template>
	<view>
		<view style="display: flex;line-height: 80rpx;background-color: #fff;width: 750rpx;padding: 31rpx;margin: 0;">
			<u-search placeholder="根据用户名搜索送货单" :show-action="false" @search="getdata"></u-search>
		</view>
		<view class="" v-if="list.length ==0" style="text-align: center;line-height: 100rpx;color: #aaa;">暂无相关送货单数据</view>
		<view>
					...
					<uni-table stripe emptyText="暂无更多数据">
						...
						<uni-tr>
							...
							<uni-td>
								...
								<u-button style="background-color: #007aff;color: #fff;height: 60rpx;width: 150rpx;" v-if="item.isservice == 0" @click="sendarr(item.id)">确认送达</u-button>
								<u-button style="background-color: #90cfff;color: #fff;height: 60rpx;width: 150rpx;" v-if="item.isservice == 1">已送达</u-button>
							</uni-td>			
						</uni-tr>
					</uni-table>
				</view>
		...
		methods: {
			sendarr(id){
				...
				uni.showModal({
					...
					success(res) {
						if(res.confirm){
							that.$get("/sendarr?id="+id).then(res=>{
								this.getdata("")
							})
						}
					}
