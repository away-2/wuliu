				...
				<view class="" slot="body">
					<uni-table stripe emptyText="暂无更多数据">
						...
						<uni-tr>
							...
							<uni-td>
								<u-button style="background-color: #007aff;color: #fff;height: 60rpx;width: 150rpx;"@click="zhuanji(item)">转寄</u-button>
							</uni-td>			
						</uni-tr>
					</uni-table>
				</view>
...
<script>
	export default {
		...
		methods: {
			zhuanji(item){
				uni.setStorageSync("outstorzhuan",item)
				uni.navigateTo({
					url: 'zhuanji/zhuanji'
				});
			},
			updata(item){
				uni.setStorageSync("outstor",item)
				uni.navigateTo({
					url: 'updata'
				});
			},
			del(id){
					...
					uni.showModal({
						...
						success(res) {
							if(res.confirm){
								that.$get("/delsending?id="+id).then(res=>{
									that.getdata("")
									uni.showToast({
										title:'删除成功',
										icon:'none'
